...
2. **性能基线与节点缓存协调（进行中）**
   ...
   - 当前：已加入前端分段耗时记录、轻量节点签名和同浏览器多标签页单任务协调；服务端共享节点数据缓存尚未开始。
3. **Templates 延迟复测与解耦**
   - 区分接口、JSON 解析、预览和浏览器主线程阻塞耗时。
   - 节点后台更新不得阻塞 Templates 首屏；大型模板按需生成预览。
   - 当前：已在测试包的 2 个模板环境完成验证：空闲预取请求 128.1ms、数据整理 0.2ms、首次渲染 2.6ms；Templates 保持活跃期间未触发 `/object_info` 刷新。主包复测仍是发布门槛。
4. **Workflows2 增量更新**
...
5. **面板信息架构与玻璃效果**
   ...
   - 修复玻璃背景层级、透明度与模糊滑块联动。
   - 当前：“打开”/“浏览”已支持折叠并保存状态；节点一级分区也已支持折叠，并共用分区标题结构。工作流两区已改为共同内容容器的相邻分区，顶部容器不再位于两区之间，因此不再引入顶部底边框；节点分区保持现状。背景设置已改为“透明背景 / 磨砂玻璃”互斥单选；毛玻璃使用固定模糊、雾面层和5–95透明度控制。标题图标、主题色和留白密度仍待最终视觉验收。
6. **工程化与发布准备**
