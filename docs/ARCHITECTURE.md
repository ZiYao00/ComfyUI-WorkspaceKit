...
| Workflows | `workflows/official-adapter.js` | Narrow ComfyUI official workflow Store/command compatibility adapter. Official-root open and active-close transitions delegate to ComfyUI commands; when ComfyUI starts with a painted graph but `activeWorkflow=null`, the adapter may register that existing graph as an official temporary workflow solely to create a navigation anchor. The adapter must not own graph loading or a workflow load queue. |
...
6. **Settings and sidebar shell — in progress.** `settings/controls.js` owns settings-dialog control DOM, `dialog-sections.js` composes the current sections through injected values/actions, and `dialog-shell.js` creates the backdrop/header DOM plus a close-intent callback. The next settings batch follows a domain-based information architecture (workflows, nodes, templates, groups, shortcuts, appearance, extensions, data and maintenance, about); group pointer gestures remain under Shortcuts, while group representation/conversion gets its own Groups page. Keep localStorage reads/writes, node-cache implementation, background-material and glass-overlay behavior, dialog attach/remove lifecycle, version request, and global keyboard handlers in the entry until separately evidenced; those paths have previously affected sidebar placement.
...
- Official-root workflow lifecycle is ComfyUI-owned: WorkspaceKit may route target intent and keep presentation-only pending/dirty state, but it must not call `workflow.load()` / `app.loadGraphData()` to open official tabs or maintain a parallel workflow-load queue. The one bootstrap exception is Store registration only: if the Store has no active/open workflow while a graph is already painted, WorkspaceKit may serialize that current graph into `createTemporary()` and call Store `openWorkflow()` so the next transition can immediately return to the official workflow-service command path.
