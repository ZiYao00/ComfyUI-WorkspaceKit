...
- Persistence boundary: workflow JSON remains the primary source for saved groups. The former global bare `xzg_groups_backup` payload is now a versioned recovery envelope scoped by workflow path plus node id/type signature. Legacy bare backups are intentionally rejected because they cannot prove workflow identity. Local recovery is allowed only during the first page-load restore window; once that restore completes, later A -> B workflow switches cannot consume the boot recovery record. Native representation also clears/ignores WorkspaceKit recovery.
- Restore ordering: periodic group-to-extra/local sync is gated by `_restoreReady`, preventing a transient startup `groups={}` from erasing recovery data. `_restoreAfterLoad` performs a bounded identity wait because ComfyUI can configure the graph before its Workflow Store exposes a stable path. It never weakens the scope match merely to make recovery succeed.
- New contracts: `entry/canvas-groups/persistence-policy.js` isolates recovery-envelope parsing and scope matching; `scripts/test-group-persistence-policy.mjs` covers same-workflow recovery, workflow mismatch, node-signature mismatch, and rejection of the legacy unscoped payload. `scripts/e2e/canvas-group-natural-refresh.mjs` exercises the production path without manually calling `Workspace2CanvasGroups.init()` or `app.loadGraphData()`: create a group on the loaded graph -> `page.reload()` -> verify the exact group id, title, node ids, graph signature, overlay DOM, and restore-ready state.
- Validation: the natural-refresh E2E passed on `:8190`; the complete JavaScript contract runner passed **117/117** tests; frontend module-goal syntax passed over **160 files**; `t035-rgthree-pale-blue-disk-persistence.mjs` passed with both WorkspaceKit groups restored; `t102-conversion-disk-persistence.mjs` passed with Native representation retained and zero WorkspaceKit overlay groups. `p0-canvas-group-reload-nodes2.mjs` also passed after replacing its obsolete 15-second startup assumption with the same bounded natural-init wait used by current real-page tests; Nodes 2.0 preserved title, bounds, node ids, node positions, and one overlay group across its in-memory reload. The pre-change source rollback archive is `.codex-backups/10-ui-canvas/ComfyUI-WorkspaceKit-before-group-refresh-persistence-fix-20260922-211513.zip`.


## 2026-09-19 - Official workflow lifecycle delegation

- User-facing defect: frequent switching in Workflows > Open could eventually stop responding and leave `Cannot read properties of undefined (reading 'path')` in the panel footer. The old implementation serialized official workflow changes through a WorkspaceKit-owned `workflowOpenQueue`, called `workflow.load()` itself, and then called `app.loadGraphData()` to imitate the official service.
- Architecture boundary: official-root open transitions now route through `workflows/official-adapter.js`. The adapter exposes the target as an official background tab, temporarily places it immediately after the active tab, invokes the public `Workspace.NextOpenedWorkflow` command, and restores the normal tab order immediately. The command captures the adjacent target synchronously and then delegates the actual load to ComfyUI `workflowService.openWorkflow()`, so ComfyUI owns the load queue, active-workflow transition, ChangeTracker lifecycle, navigation intent, failure recovery, and graph replacement.
- WorkspaceKit retains only presentation intent (`workflowOpenUiRequestId` / `pendingWorkflowPath`) and semantic Save-indicator state. Those values may control WK pending/dirty UI but cannot cancel, serialize, or perform official graph loads. Official-root failure now fails closed instead of falling back to a second `app.loadGraphData()` lifecycle. Non-official custom roots keep their separate WorkspaceKit file-loading path.
- Save/close boundary: Workflows > Open Save now uses the same official `Comfy.SaveWorkflow` path as the floating top-bar Save. Active official tabs close through `Workspace.CloseWorkflow`; inactive tabs require no canvas transition and use only the official Store detach after WK has no pending navigation and after its existing dirty save/discard policy completes.
- Runtime probes: `:8188` is ComfyUI `0.36.0` / frontend `1.52.7`; `:8190` is the test package on ComfyUI `0.34.0` / frontend `1.52.7`. The 8190 Store starts with `activeWorkflow=null` and `openWorkflows=[]` even though the default graph is already painted. That state caused the first implementation to return `official-navigation-state-mismatch`. The compatibility adapter now registers the current painted graph through official `createTemporary()` + Store `openWorkflow()` as `workflows/Unsaved Workflow.json`, then immediately delegates the requested persisted target to `Workspace.NextOpenedWorkflow`. A focused 8190 probe confirmed the command captured `workflows/New Workflow.json` while preserving the restored tab order. A production-adapter headless probe also confirmed the old state-mismatch no longer occurs; the remaining headless graph load stopped later inside the 8190 third-party extension chain with `getCanvas: canvas is null`, so final headed-browser acceptance must be performed in the normal 8190 UI rather than being inferred from that automation failure.
- Automated validation: source backups exist both before the lifecycle-delegation batch and before the 8190 bootstrap fix (`20-workflows/ComfyUI-WorkspaceKit-before-before-8190-bootstrap-fix-20260919-130428.zip`). `scripts/test-workflow-official-navigation.mjs` now covers `active=null/open=[]` bootstrap, active-but-not-open re-anchoring, rapid B/C target capture, immediate tab-order restoration, active official close command delegation, and inactive tab detach. `scripts/test-workflow-switch-observability.mjs` forbids the retired WK load queue and direct official-root graph loading. After the bootstrap fix, the repository-wide JavaScript contract runner again passed **115/115** tests, frontend syntax covered **158 files**, and release-version check passed at **0.2.6**; the existing Python contracts were already green at **9/9** in the same batch. `scripts/e2e/workflow-switch-queue.mjs` performs repeated A/B/C cycles (12 rounds by default) and verifies the final active workflow, continued switchability, cleared pending UI, and absence of the historical `undefined.path` error.
...
## 2026-08-29 — L1-A1 Layout position memory + familiar icon baseline

- Scope is intentionally limited to the Layout command surface. Theme and Settings presentation-mode migration are not part of this batch.
- The eight high-frequency alignment/distribution commands now have an explicit fixed 4×2 contract: row 1 is left / horizontal-center / right / horizontal-distribute; row 2 is top / vertical-center / bottom / vertical-distribute. Numeric spacing and size operations remain separate below that position-memory block.
- The full Layout panel and canvas top bar continue to call the same command registry and `createLayoutCommandIcon()` implementation. For those primary eight commands, `entry/layout/icons.js` now restores the familiar SVG path shapes from the audited GPL-3.0 ComfyUI-NodeAligner baseline at commit `321ec9dcb859404f4b89cbd359ebc2c25ac59146`, recolored through `currentColor` rather than copying Adobe assets.
- `scripts/test-layout-integration.mjs` now locks the exact two-row command order and icon provenance. Focused evidence passed: Layout Integration, Layout Engine, and Layout Transaction + mixed Group contracts; the final whole-repository JavaScript regression also passed **109/109** contracts, including **151** frontend module syntax checks.
- Real ComfyUI visual acceptance is still required for icon familiarity, perceived spacing, the 4×2 matrix at normal/narrow sidebar widths, and the shared top-bar appearance. L1-A2 will separately move persistent presentation preferences into Settings and leave only a small `显示方式` entry in the Layout panel.

## 2026-08-29 — L1-A1.1 + L1-A2 Layout UI/presentation batch

- L1-A1.1 restores the remaining four historical NodeAligner max/min width/height SVGs, so all twelve commands that existed in the old Layout command set now use the familiar audited icon vocabulary. Fixed-spacing and equal-both did not exist in that historical icon set and remain clearly identified as WorkspaceKit-owned supplemental SVGs.
- The Layout palette is regularized: the primary commands are a full-width 4×2 matrix, size commands are one equal five-column row, and the spacing number + horizontal/vertical spacing actions moved into the upper control strip with one accent treatment instead of forming a third command block.
- L1-A2 removes the persistent topbar checkbox from the operational palette. The panel now keeps only `显示方式` / `Display mode`; Settings owns four mutually exclusive presentation modes: top toolbar (fresh-install default), show after selection, pinned draggable canvas toolbar, or none. Icon size is persisted at 18–25 px, and the floating position can be reset.
- The selection/pinned toolbar is a new WorkspaceKit surface using the same eight high-frequency command registry and SVG factory. It does not restore the old NodeAligner Shadow DOM or permanent polling implementation. Old Layout/NodeAligner presentation mode, icon-size, and position preferences are migrated non-destructively when present.
- Focused contracts passed: Layout Integration/presentation/icon provenance, Layout Engine, Layout Transaction + mixed Group, and Settings dialog sections. Whole-repository regression passed **109/109 JavaScript contracts**, **9/9 Python contracts**, and frontend module syntax passed **154 files**.
- Real ComfyUI acceptance is still pending for the colored spacing group, 4×2/5-column visual rhythm, all restored icons, `显示方式` navigation, each of the four display modes, toolbar drag/reset, and live icon-size changes. A dedicated `scripts/e2e/l1-layout-presentation.mjs` smoke was added, but the current `:8190` headless environment only returns HTTP 200 and never exposes `.workspace2-tab-button` within 45 seconds. The existing L0 real-page smoke fails at the same pre-WorkspaceKit selector, so no L1-A2 functional assertion was reached; this is recorded as a test-environment gate rather than a feature pass/fail. Automated evidence must not be treated as visual/interaction acceptance.
