...
| B10 | Layout UI / 共享 Shell 接入主仓 UI Kit | **L0 + L1-A1/A1.1/A2 code/contract done 2026-08-29；real-page gate pending** | L0 已完成共享壳层与 5+1 标签控制。L1-A1/A1.1 将 8 个高频命令固定为全宽 4×2，并将 5 个尺寸命令整理为一行等分；间距数值框 + 横/竖固定间距移到顶部并使用同一强调色。历史 NodeAligner SVG 已恢复到 12 个旧命令（8 对齐/分布 + 4 最大/最小宽高），固定间距与统一最大尺寸为 WK 自有补充图标。L1-A2 已把持久显示偏好迁到 Settings：顶部 / 选中后 / 画布悬浮驻留 / 不显示，支持 18–25 px 图标大小与悬浮位置重置；排版页只保留「显示方式」入口。新 selection/pinned 浮动条共用统一 Command Registry，没有恢复旧 Shadow DOM/永久轮询。 |
...
| B12 | 单插件总回归与文档收口 | **in-progress 2026-08-29** | L1-A1.1/A2 后整仓回归继续为 109 项 JS contract、9 项 Python contract 全部通过，frontend module syntax 随 3 个新增 Layout 模块增至 154 个文件并全绿。自动合同已覆盖 4×2 位置记忆、12 个历史 SVG 来源、A2 四种显示模式的偏好/迁移、Settings 接线与统一 Command Registry；真实 ComfyUI 仍需人工验收新面板节奏、间距强调组、显示方式跳转、顶部/选中/驻留/关闭四种模式、悬浮拖动/复位与图标大小。产品路线保持 Layout → Theme/Appearance → WK Core。 |
