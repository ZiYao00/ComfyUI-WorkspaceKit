...
| B08 | Layout Node 命令逐项迁移 | **done 2026-08-30** | Align / Distribution / Size / Fixed Spacing 已进入 Engine；普通/折叠节点、undo/redo、临时工作流真实保存→重启→重读均已完成真实页面验证。 |
| B09 | Groups × Layout 内部适配 | **done 2026-08-30** | Group LayoutTarget adapter 与 Transaction 混合 Group contract 均通过；真实页面 Group+Group、Node+Group 对齐/分布路径已验收。Post-L1 进一步将 Layout/Groups 节点视觉 Bounds 统一到共享 resolver，并补齐空白编组、多选拖动、联合拖动、Resize 的最终成员 reconciliation。 |
| B10 | Layout UI / 共享 Shell 接入主仓 UI Kit | **done 2026-08-30** | L0 共享壳层与 5+1 标签控制、L1-A1/A1.1 的 4×2 高频命令/5 项尺寸/间距强调组、12 个历史 SVG，以及 L1-A2 的顶部/选中后/画布悬浮驻留/不显示四种模式均已完成。`l1-layout-presentation` 在 8190 真实页面通过；Sidebar 入口选择器与 🧩 图标已改为稳定 tab identity，不再依赖 ComfyUI 重绘后可能丢失的旧 class。 |
...
| B12 | 单插件总回归与文档收口 | **in-progress 2026-08-30** | Layout/Post-L1 收口后的基线为 109 项 JS contract、9 项 Python contract、155 个 frontend module syntax 全绿，版本检查 0.2.6 通过；8190 上 `l1-layout-presentation`、Sidebar smoke、`post-l1-group-regressions` 均通过。Layout 产品线已关闭，B12 保持 in-progress 仅因为统一插件还要继续完成 Theme / Appearance 与随后 Core 的总回归/文档收尾。产品路线保持 Layout → Theme/Appearance → WK Core。 |
